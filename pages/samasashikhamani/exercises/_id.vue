...
export default {
  ...
  mounted() {
    // console.log(this.id)
    this.exercise = this.$store.state.samasashikhamani_exercises.find(
      ex => String(ex.id) === this.id
    )
  },
  created() {
    // console.log(this.$store.state.exercises.find(exercise => exercise.id === '4.1'))
    this.index = this.$store.state.samasashikhamani_exercises.findIndex(
      ex => String(ex.id) === this.id
    )
    this.prev_ex = this.$store.state.samasashikhamani_exercises[this.index - 1]
    this.next_ex = this.$store.state.samasashikhamani_exercises[this.index + 1]
    this.length = this.$store.state.samasashikhamani_exercises.length
    this.previous =
      this.index > 1
        ? '/samasashikhamani/exercises/' + this.prev_ex.id
        : '/samasashikhamani/exercises/1'
    this.next =
      this.index < this.length - 1
        ? '/samasashikhamani/exercises/' + this.next_ex.id
        : '/samasashikhamani/exercises/10'
